...
const isChefOrManager = (req, res, next) => {
  // console.log("in isServerOrManager");
  // console.log(req.headers.userrole);
  if (req.headers.userrole !== 'Chef' && req.headers.userrole !== 'Manager') {
    return res.status(403).send({ message: 'Require Chef or Manager Role!' });
  }
  next();
};
...
export { verifyToken, isManager, isServer, isChef,isServerOrManager,isChefOrManager };